...
[Serializable]
public class GridMaterialPiece  {
    public GridPiece gridPiece;
    [SerializeField] public Block material;
    [SerializeField] public int data;
    private MaterialShape shape;

    public GridMaterialPiece(Block material, GridPiece gridPiece) {
        this.material = material;
        this.gridPiece = gridPiece;
        this.shape = GridMaterial.I[material].shape;
    }
    ...
    public void Render() {
        ...
        for (int i = 0; i < 3; i++) {
            int value = 0;
            if (GridMaterial.I[material].ShapeAt(hc[i] - GridPos.up)) value += 1;
            if (GridMaterial.I[material].ShapeAt(hc[i])) value += 2;
            if (GridMaterial.I[material].ShapeAt(hc[i] + GridPos.up)) value += 4;
            data[i] = value;
        }
        ...
        if ((data[0] & 2) * (data[1] & 2) * (data[2] & 2) == 8) {
            if ((data[0] & 4) + (data[1] & 4) + (data[2] & 4) == 0) {
                Create(shape.floor, level, Random.Range(0, 3), Randoms.CoinFlip, false, Four(3, 4, 5));
            }
            if ((data[0] & 1) + (data[1] & 1) + (data[2] & 1) == 0) {
                Create(shape.floor, level, Random.Range(0, 3), Randoms.CoinFlip, true, Four(3, 4, 5));
            }
            return;
        }
        ...
        if(FindSlope(data,
            ...
            if (extraCornerValue == -1) { // Something is symmetric
                if (top2 != -1) {
                    if (lowerBase1 != -1) {
                        Create(shape.upperSlope, level, lowerBase1, Randoms.CoinFlip, false, Four(3, 1, 5));
                    } else {
                        Create(shape.upperSlope, level, upperBase1, Randoms.CoinFlip, true, Four(3, 1, 5));
                    }
                } else if (lowerBase1 != -1 && upperBase1 != -1) {
                    bool reflect = upperBase1 == NextCorner(top1);
                    bool rotateOnZ = Randoms.CoinFlip;
                    Create(shape.tunnelSlopeDouble, level, top1, reflect ^ rotateOnZ, rotateOnZ, Four(6, 4, 2));
                } else {
                    Create(shape.lowerSlope, level, top1, Randoms.CoinFlip, upperBase2 != -1, Four(0, 4, 2));
                }
            } else {
                ...
                if (extraCornerValue == 0) {
                    Create(topIsThin ? shape.tunnelSlopeLedge : shape.upperCurve,
                        level, base1, !baseIsAfterTop, yFlip, Four(3, 1, 0));
                } else if (extraCornerValue == 4) {
                    Create(shape.tunnelCurve, level, base1, !baseIsAfterTop, yFlip, Four(3, 1, 8, 0));
                } else if (extraCornerValue == 5) {
                    Create(shape.tunnelSlope, level, base1, !baseIsAfterTop, yFlip, Four(3, 1, 8, 2));
                } else { // extraCornerValue == 6 || extraCornerValue == 7
                    Create(shape.lowerCurve, level, top1, baseIsAfterTop, yFlip, Four(0, 4, 5, 8));
                }
            }
            return;
        }
    ...
    private void RenderHalf(int level, int[] data, bool yFlip) {
        ...
        if (HasTwo(data, 0, ref otherLoc, ref otherValue)) {
            if (otherValue == 1) {
                Create(shape.revcornerGutter, level, otherLoc, Randoms.CoinFlip, yFlip, Four(4));
            } else { // if (otherValue == 3)
                Create(shape.revcorner, level, otherLoc, Randoms.CoinFlip, yFlip, Four(4, 7));
            }
        } else if (HasTwo(data, 1, ref otherLoc, ref otherValue)) {
            if (otherValue == 0) {
                Create(shape.cornerGutter, level, otherLoc, Randoms.CoinFlip, yFlip, Four(5, 3));
            } else { // if (otherValue == 2)
                Create(shape.cornerMoulding, level, otherLoc, Randoms.CoinFlip, yFlip, Four(3, 7, 5));
            }
        } else if (HasTwo(data, 2, ref otherLoc, ref otherValue)) {
            Create(shape.revcornerMoulding, level, otherLoc, Randoms.CoinFlip, yFlip, Four(6, 4, 8));
        } else if (HasTwo(data, 3, ref otherLoc, ref otherValue)) {
            if (otherValue == 0) {
                Create(shape.corner, level, otherLoc, Randoms.CoinFlip, yFlip, Four(5, 3, 8, 6));
            } else { // if (otherValue == 2)
                Create(shape.cornerMoulding, level, otherLoc, Randoms.CoinFlip, yFlip, Four(3, 7, 5));
            }
            ...
            if (sum == 3) {
                Create(shape.tunnelPillarSlant, level, pivot, xFlip, yFlip, Four(8, 3));
            } else if (sum == 4) {
                Create(shape.endGutter, level, pivot, xFlip, yFlip, Four(5, 4, 8));
            } else if (sum == 5) {
                Create(shape.endMoulding, level, pivot, xFlip, yFlip, Four(3, 7, 6));
            } else {
    ...
    public Transform Create(GameObject prefab, int y, int pivot, bool xFlip, bool yFlip, int[] origSources) {
        ...
        Transform newPiece = GameObject.Instantiate(prefab, gridPiece.transform).transform;
        newPiece.localRotation = Quaternion.Euler(0, yRot, 0);
        newPiece.localScale = Vector3.Scale(new Vector3(xFlip ? -1 : 1, yFlip ? -0.5f : 0.5f, 1), WorldGrid.I.scale);
    ...
    private void SetMaterial(Transform newPiece, int[] src) {
        ...
        Material material = new Material(WorldGrid.I.defaultMaterial);
    ...
    private Color GetFloorColor(int sourceId) {
        return Color.green;
        // if (sourceId == -1) return WorldGrid.Biome.DefaultFloor;
        // int corner = sourceId % 3;
        // int yOffset = sourceId / 3 - 1;
        // return WorldGrid.Biome.GetFloor(pos.HorizCorners[corner] + GridPos.up * yOffset);
    }
    private Color GetWallColor(int sourceId) {
        return Color.gray;
        // if (sourceId == -1) return WorldGrid.Biome.DefaultWall;
        // int corner = sourceId % 3;
        // int yOffset = sourceId / 3 - 1;
        // return WorldGrid.Biome.GetWall(pos.HorizCorners[corner] + GridPos.up * yOffset);
    }
}
